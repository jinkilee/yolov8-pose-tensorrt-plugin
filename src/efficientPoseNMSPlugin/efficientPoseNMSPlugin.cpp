...
using nvinfer1::plugin::EfficientPoseNMSONNXPluginCreator;

namespace
{
char const* const kEFFICIENT_NMS_PLUGIN_VERSION{"1"};
char const* const kEFFICIENT_NMS_PLUGIN_NAME{"EfficientPoseNMS_TRT"};
char const* const kEFFICIENT_NMS_ONNX_PLUGIN_VERSION{"1"};
char const* const kEFFICIENT_NMS_ONNX_PLUGIN_NAME{"EfficientPoseNMS_ONNX_TRT"};
} // namespace
    ...
    PluginTensorDesc const* inputs, int32_t nbInputs, PluginTensorDesc const* outputs, int32_t nbOutputs) const noexcept
{
    ...
    return EfficientPoseNMSWorkspaceSize(batchSize, numScoreElements, numClasses, mParam.datatype);
}
...
EfficientPoseNMSONNXPluginCreator::EfficientPoseNMSONNXPluginCreator()
    : mParam{}
{
    mPluginAttributes.clear();
    mPluginAttributes.emplace_back(PluginField("score_threshold", nullptr, PluginFieldType::kFLOAT32, 1));
...
char const* EfficientPoseNMSONNXPluginCreator::getPluginName() const noexcept
{
    return kEFFICIENT_NMS_ONNX_PLUGIN_NAME;
}

char const* EfficientPoseNMSONNXPluginCreator::getPluginVersion() const noexcept
{
    return kEFFICIENT_NMS_ONNX_PLUGIN_VERSION;
}

PluginFieldCollection const* EfficientPoseNMSONNXPluginCreator::getFieldNames() noexcept
{
    return &mFC;
}

IPluginV2DynamicExt* EfficientPoseNMSONNXPluginCreator::createPlugin(
    char const* name, PluginFieldCollection const* fc) noexcept
{
    try
    {
        gLogWarning << "EfficientPoseNMSONNXPlugin is deprecated since TensorRT 9.0. Use INetworkDefinition::addNMS() to "
                       "add an INMSLayer."
                    << std::endl;
        PluginField const* fields = fc->fields;
        for (int32_t i = 0; i < fc->nbFields; ++i)
...
IPluginV2DynamicExt* EfficientPoseNMSONNXPluginCreator::deserializePlugin(
    char const* name, void const* serialData, size_t serialLength) noexcept
{
    try
    {
